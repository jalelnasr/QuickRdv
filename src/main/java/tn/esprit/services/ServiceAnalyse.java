...
public class ServiceAnalyse implements IService<Analyse> {
    ...
    public void add(Analyse analyse) {
        String qry = "INSERT INTO `demande_analyse`(`date`, `type`, `instructions`, `id_patient`, `id_medecin`) VALUES (?, ?, ?, ?, ?)";
        try {
    ...
    public List<Analyse> getAll() {
        ...
        String qry = "SELECT da.id, da.date, da.type, da.instructions, da.id_patient, da.id_medecin, " +
    ...
    public void update(Analyse analyse) {
        String qry = "UPDATE `demande_analyse` SET `date` = ?, `type` = ?, `instructions` = ?, `id_patient` = ?, `id_medecin` = ? WHERE `id` = ?";
        try {
            PreparedStatement pstm = cnx.prepareStatement(qry);
            pstm.setTimestamp(1, new java.sql.Timestamp(analyse.getDate().getTime()));
            pstm.setString(2, analyse.getType());
